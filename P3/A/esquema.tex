...
De este diseño destacar que la dirección se encuentra almacenada un objeto
JSON anidado en la tabla, en vez de estar referenciado a una tabla distinta
ya que este posee poco tamaño y no va a sufrir muchos cambios. Asimismo, se
...
único del usuario). Además, se considera crear un índice sobre este campo,
para que así, acceder a todas las respuestas de un usuario solo involucre
una búsqueda sobre ese campo, sin necesidad de realizar consultas a otras
colecciones. 

Aunque los votos se encuentren en una colección distinta, se ha decidido que
cada respuesta posea una cuenta de votos positivos y negativos que ha
recibido. De esta manera, acceder a una respuesta y consultar el número de
votos se realiza de manera inmediata.

Para guardar los comentarios se ha decidido anidar en un array de
comentarios por cada pregunta. Esta decisión ha sido tomada principalmente
por dos motivos: (a) Nunca se va a acceder a un comentario directamente,
sino que se realizará a través de la respuesta. De esta manera, acceder a
...
ser pocos y pequeños (similar a StackOverflow), por lo que no supondrá una
disminución de la eficiencia.

En esta colección también se ha decidido incorporar una referencia al id de
...
permite acceder al título de la pregunta original y almacenarlo con el
nuevo voto, para así agilizar las consultas posteriores a los votos.
...
Para almacenar las votaciones se ha decidido utilizar una colección que
solo almacene estas, ya que los usuarios van a generar un gran número de
estas, su número es ilimitado, y además pueden sufrir cambios. Junto con la
votación (representada por un número entero), se almacena el id de la
respuesta a la que corresponde la votación, el alias del usuario que la ha
realizado (identificador único del usuario), y el título de la pregunta
original.

El alias del usuario posee un índice sobre su campo para así agilizar las
consultas de obtener todas las votaciones de un único usuario. Como el
título de una pregunta no se va a modificar, y para agilizar las consultas
sobre votaciones, se ha decidido guardar una copia en esta colección,
evitando así tener que consultar otras colecciones para obtener los datos.

Un ejemplo de documento en esta colección es el siguiente:
